...
from ragpill._text import normalize_text


def testnormalize_text_nfkc_and_whitespace():
    # Collapses whitespace and normalizes Unicode (UF₆ -> UF6)
    assert normalize_text("  UF₆\n  sample\t") == "uf6 sample"
    assert normalize_text("Another     document \n\n\t with different") == "another document with different"


def testnormalize_text_preserves_plain_text():
    assert normalize_text("Simple text") == "simple text"
...
def testnormalize_text_strips_single_tilde_subscripts_up_to_10_chars():
    assert normalize_text("UF~6~") == "uf6"
    assert normalize_text("UF~1234567890~") == "uf1234567890"


def test_normalize_quotes():
    assert normalize_text('He said, "This is a quote."') == normalize_text("he said, 'this is a quote.'")
    assert normalize_text("beta labs\u2019 past sample activities") == normalize_text(
        "Beta Labs\u2019 past sample\n\n    activities."
    )


def testnormalize_text_leaves_long_tilde_segments():
    assert normalize_text("UF~12345678901~") == "uf~12345678901~"


def testnormalize_text_adds_case_insensitive_and_nfkc():
    normalized = normalize_text("UF₆ status")
    assert normalized == "uf6 status"


def testnormalize_text_preserves_existing_case_flag():
    normalized = normalize_text("Error|Failure")
    assert normalized == "error|failure"


def testnormalize_text_trims_and_collapses_spaces():
    normalized = normalize_text("   foo   bar   ")
    assert normalized == "foo bar"


def testnormalize_text_starts_with_multiline_only():
    normalized = normalize_text("(?m)^start")
    assert normalized == "(?m)^start"


def testnormalize_text_with_existing_si_flags():
    normalized = normalize_text("(?si)Already")
    assert normalized == "(?si)already"


def testnormalize_text_with_literal_parentheses():
